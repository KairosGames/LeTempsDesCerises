@tool
class_name LineRenderer3D extends MeshInstance3D
...
func _process(_delta):
	...
	for i in range(points.size() - 1):
		...
		var colA : Color = start_color.lerp(end_color, progress)
		var colB : Color = start_color.lerp(end_color, progress + progressStep)
		
		if i == 0:
			if draw_caps:
				cap(A, B, thickness, cap_resolution, colA)
		...
		if i == points.size() - 2:
			if draw_caps:
				cap(B, A, nextThickness, cap_resolution, colB)
		else:
			if draw_corners:
				var C = points[i+2]
				if use_global_coords:
				...
				if angleDot > 0 and not angleDot == 1:
					corner(B, BtoABEnd, B + orthogonalBCStart, corner_resolution, colB)
				elif angleDot < 0 and not angleDot == -1:
					corner(B, B - orthogonalBCStart, BfromABEnd, corner_resolution, colB)
...
func cap(center:Vector3, pivot:Vector3, thickness:float, cap_resolution:int, col:Color):
...
func corner(center:Vector3, start:Vector3, end:Vector3, cap_resolution:int, col:Color):
	var vertex_array:Array = []
	for i in range(cap_resolution + 1):
		vertex_array.append(Vector3(0,0,0))
	...
	var v0: Vector3 = start - center
	var v1: Vector3 = end - center
	var axis_raw: Vector3 = v0.cross(v1)
	
	if axis_raw.length_squared() < 1e-8: return
...
func get_thickness_at_point(index: int) -> float:
	if points.is_empty():
		return start_thickness
	var t := float(index) / float(max(points.size() - 1, 1))
	return lerp(start_thickness, end_thickness, t)
