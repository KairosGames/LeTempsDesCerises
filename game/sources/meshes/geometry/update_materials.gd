...
func _run() -> void:
	...
	for mesh_file: String in DirAccess.get_files_at(MESH_DIR):
		...
		_create_variant(mesh, name)
		
		if MESH_VARIANTS.has(name):
			for variant_name: String in MESH_VARIANTS[name]:
				_create_variant(mesh, variant_name)
	
func _create_variant(original_mesh: ArrayMesh, name: String) -> void:
	...
	for index: int in range(mesh.get_surface_count()):
		...
		if existing:
			if existing.surface_get_name(index) == surface_name:
